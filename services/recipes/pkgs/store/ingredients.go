...
func AddIngredients(recipeId string, ingredientNames []string) error {
	...
	for index, name := range ingredientNames {
		...
		fmt.Printf("Adding %s\n", name)
		err := insertInto(db, "ingredients", []string{"id", "name"}, []Record{ingredient})
		if err != nil {
	...
	err := insertInto(db, "ingredients_recipes", []string{"ingredient_id", "recipe_id"}, ingredientRecipes)
...
func GetIngredient(name string) (Ingredient, error) {
	...
	row := db.QueryRow("SELECT id, name FROM ingredients WHERE name = ?", name)
